...
var MAX_MATCHES = 15;
var LitAutocomplete = class extends s4 {
  ...
  get contentElement() {
    ...
    var slotInputList = this.shadowRoot.getElementById("dropdown-input").assignedNodes()[1];
    this._inputEl = slotInputList ? slotInputList : this.shadowRoot.getElementById("defaultInput");
    return this._inputEl;
  }
  set fulllist(value) {
    this.items = value;
    ...
    switch (ev.key) {
      ...
      default:
        if (this.items.length) {
          ...
          suggestions = value && this.items.filter((item) => item.text.replace(",", "").replace(/\s/g, "").toLowerCase().search(value.replace(",", "").replace(/\s/g, "").toLowerCase()) != -1).slice(0, this.maxSuggestions);
  ...
  _handleItemMouseLeave(ev) {
    ...
    this._blur && setTimeout((_2) => this.close(), 500);
  }
  open() {
    console.log("open()");
  ...
  autocomplete(value, text) {
    ...
    this.close();
    this.dispatchEvent(new CustomEvent("selected-autocomplete", {
      detail: { value, text },
      composed: true,
      bubbles: true
    }));
  ...
  render() {
    return p`
      ...
      >
        ${this._matches.map((item) => p`
            <li
              @click=${(ev) => this.autocomplete(item.text, item.value ? item.value : null)}
            >
              ${item.text}
            </li>
          `)}
